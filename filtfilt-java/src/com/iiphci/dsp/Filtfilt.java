...
/**
 * Change Log.
 * Minor change from chinese to english for documentation and error messages, otherwise left untouched.
 * Richard Burgmann 18/04/2019
 */
...
public class Filtfilt {
	 ...
	 *            Filter Parameter A
	 * @param input
	 *            The array to be filtered ：The requirement is one dimension，Length is greater than B array。
	 * @return Filter Result Returns double array.
	 */
	public static ArrayList<Double> doFiltfilt(ArrayList<Double> B,
		...
		if (len <= nfact)
			...
			throw new RuntimeException("The input value X is too small, and the data is at least three times the filter order.");
		resize(B,nfilt, 0);
		resize(A,nfilt, 0);
	...
	public static void filter(ArrayList<Double> B, ArrayList<Double> A,
		...
		if (flagA) {
			...
			throw new RuntimeException(" A The array must have at least one number that is not zero.");
		}
		if (A.get(0) == 0) {
